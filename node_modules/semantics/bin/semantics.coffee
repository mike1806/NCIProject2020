...
switch word
  ...
  when "-f"
    ...
    if opts.all or opts.abbrevs
      keys = util.keys(data.ru.abbrevs)
      if keys.length > 0
    ...
    if opts.all or opts.contacts
      keys =  util.keys(data.misc.phones)
      keys.push k for k in util.keys(data.misc.emails)
      if keys.length > 0
    ...
    if opts.all or opts.proper_names
      if util.keys(data.meaning.proper_names).length > 0
